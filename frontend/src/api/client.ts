...
api.interceptors.request.use(async (config) => {
  ...
  if (token) {
    ...
    console.warn("Axios interceptor: No Supabase session token found!")
  }
    ...
    if (error.response?.status === 401 && !orig._retry) {
      ...
      if (refresh) {
        try {
          ...
          orig.headers.set('Authorization', `Bearer ${data.access_token}`)
          return api(orig)
        } catch {
          localStorage.clear()
...
export const detectionApi = {
  ...
  submitFeedback: (id: string, is_accurate: boolean) =>
    api.post(`/detection/${id}/feedback`, null, { params: { is_accurate } }).then(r => r.data),
}

...
const SYSTEM_PROMPT = `You are the Vocaria AI assistant — a helpful, knowledgeable guide for the Vocaria platform. 

Vocaria is an enterprise voice AI platform with three main pillars:
...
export default function ChatbotWidget() {
  ...
  const [messages, setMessages]   = useState<Message[]>([{
    id: '0',
    role: 'assistant',
    content: `Hi${user?.display_name ? ` ${user.display_name.split(' ')[0]}` : ''}! 👋 I'm the Vocaria assistant. I can help you with voice cloning, TTS generation, deepfake detection, API usage, and more. What would you like to know?`,
    timestamp: new Date(),
  }])
      ...
      <AnimatePresence>
        {!open && (
          <motion.button
            ...
            whileTap={{ scale: 0.94 }}
            title="Ask the Vocaria assistant"
          >
          ...
          >
            ...
            <div style={{ padding: '12px 14px', background: 'var(--bg)', borderTop: '1px solid var(--border)', display: 'flex', gap: 8, alignItems: 'flex-end' }}>
              <input
                ...
                placeholder="Ask anything about Vocaria…"
                disabled={loading}
                style={{

              ...
              <div style={{ marginTop: 18 }}>
                ...
                <div className="flex flex-col sm:flex-row gap-4">
                  ...
                  <div className="flex sm:flex-col gap-3 justify-center">
                    <div style={{ fontSize: 13, color: 'var(--fg-4)', textAlign: 'center', fontWeight: 500 }} className="hidden sm:block">OR</div>
                    {isRecording ? (
                      <button onClick={stopRecording} className="btn w-full flex items-center justify-center gap-2 bg-red-50 text-red-600 border border-red-200 hover:bg-red-100 py-3 sm:py-0 sm:h-full rounded-xl">
                        <div className="w-2.5 h-2.5 rounded-full bg-red-600 animate-pulse" /> Stop Recording
                      </button>
                    ) : (
                      <button onClick={startRecording} className="btn w-full flex items-center justify-center gap-2 bg-white text-gray-700 border border-gray-200 hover:bg-gray-50 py-3 sm:py-0 sm:h-full rounded-xl transition-all hover:border-gray-300">
  ...
  return (
    <div style={{ padding: '11px 14px', borderRadius: 10, border: isFailed ? '1px solid rgba(220,38,38,0.3)' : '1px solid var(--border-2)', background: isFailed ? 'rgba(220,38,38,0.03)' : 'var(--bg-2)' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6 }}>
        ...
        <span style={{ fontSize: 11.5, color: isFailed ? '#dc2626' : 'var(--fg-4)' }}>{job.mode}</span>
      </div>

...
import { Wand2, Zap, Trash2, Globe, Lock, Building2, Heart, MessageSquare, Share2, ChevronLeft, Play, ExternalLink, Download, Pencil } from 'lucide-react'
...
export default function VoiceDetail() {
  ...
  const [editData, setEditData] = useState({ name: '', description: '', visibility: 'private' })
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([v, c, j, m]) => { 
        setVoice(v)
        setEditData({ name: v.name || '', description: v.description || '', visibility: v.visibility || 'private' })
        setComments(c || [])
        setCloneJobs(j.jobs || [])
        setModels(m.models || [])
      })
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            ...
            <div className="card" style={{ padding: 28, overflow: 'hidden', position: 'relative' }}>
              ...
              <div style={{ display: 'flex', gap: 18, alignItems: 'flex-start', flexWrap: 'wrap' }}>
                ...
                <div style={{ flex: 1, minWidth: 240 }}>
                  <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
                    {isEditing ? (
                      <div style={{ width: '100%', maxWidth: 400 }}>
                        <input value={editData.name} onChange={e => setEditData(p => ({...p, name: e.target.value}))} className="input" placeholder="Profile Name" style={{ marginBottom: 12, fontWeight: 700, fontSize: 18 }} />
                        <textarea value={editData.description} onChange={e => setEditData(p => ({...p, description: e.target.value}))} className="input textarea" placeholder="Description" rows={3} style={{ marginBottom: 12 }} />
                        <label style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 14, fontWeight: 600, color: 'var(--fg-2)', marginBottom: 16 }}>
                          <input type="checkbox" checked={editData.visibility === 'public'} onChange={e => setEditData(p => ({...p, visibility: e.target.checked ? 'public' : 'private'}))} />
                          Publish to Public Hub
                        </label>
                        <div style={{ display: 'flex', gap: 8 }}>
                          <button onClick={handleSaveEdit} disabled={savingEdit} className="btn btn-primary btn-sm">{savingEdit ? <Spinner size={12} /> : 'Save'}</button>
                          <button onClick={() => { setIsEditing(false); setEditData({ name: voice.name, description: voice.description, visibility: voice.visibility }) }} className="btn btn-ghost btn-sm">Cancel</button>
                        </div>
                      </div>
                    ) : (
                      <>
                        ...
                        {isOwner && (
                          <div style={{ display: 'flex', gap: 8 }}>
                            <button onClick={() => setIsEditing(true)} className="btn btn-secondary btn-sm" style={{ gap: 5 }}><Pencil size={12} />Edit</button>
                  ...
                  {!isEditing && voice.description && <p style={{ fontSize: 13.5, color: 'var(--fg-3)', lineHeight: 1.65, marginBottom: 16 }}>{voice.description}</p>}
          ...
          <Reveal delay={0.07}>
            <div className="card" style={{ padding: 24 }}>
              <div style={{ fontSize: 14, fontWeight: 600, color: 'var(--fg)', marginBottom: 18, display: 'flex', alignItems: 'center', gap: 8, justifyContent: 'space-between' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <Zap size={15} style={{ color: 'var(--fg-4)' }} />Cloned Voices ({cloneJobs.length})
                </div>
                {isOwner && (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                {cloneJobs.length === 0 && <div style={{ textAlign: 'center', padding: '20px 0', color: 'var(--fg-5)', fontSize: 13.5 }}>No cloned voices yet. Clone this profile to use it in TTS!</div>}
                {cloneJobs.map((job: any) => (
                  <div key={job.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 16px', background: 'var(--bg-2)', borderRadius: 10, border: '1px solid var(--border-2)' }}>
                    <div>
                      <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--fg-2)', marginBottom: 4, textTransform: 'capitalize' }}>Clone Job ({job.mode.replace('_', ' ')})</div>
                      <div style={{ fontSize: 11.5, color: 'var(--fg-4)' }}>{new Date(job.created_at).toLocaleDateString()} · Status: <span style={{ textTransform: 'capitalize', color: job.status === 'completed' ? 'var(--green)' : job.status === 'failed' ? 'var(--red)' : 'var(--blue)' }}>{job.status}</span></div>
                    </div>
                    {job.status === 'completed' && job.quality_score && (
                      <span className="badge badge-green">Quality: {(job.quality_score * 100).toFixed(0)}%</span>
                    )}
                  </div>
                ))}
              </div>
          ...
          {/* Generated Voices / Models */}
          <Reveal delay={0.075}>
            <div className="card" style={{ padding: 24 }}>
              <div style={{ fontSize: 14, fontWeight: 600, color: 'var(--fg)', marginBottom: 18, display: 'flex', alignItems: 'center', gap: 8 }}>
                <Wand2 size={15} style={{ color: 'var(--fg-4)' }} />Generated Voices ({models.filter(m => m.source_type === 'generated').length})
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                {models.filter(m => m.source_type === 'generated').length === 0 && <div style={{ textAlign: 'center', padding: '20px 0', color: 'var(--fg-5)', fontSize: 13.5 }}>No generated voices saved to this profile yet.</div>}
                {models.filter(m => m.source_type === 'generated').map((model: any) => (
                  <div key={model.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 16px', background: 'var(--bg-2)', borderRadius: 10, border: '1px solid var(--border-2)' }}>
                    {model.preview_url ? (
                      <audio controls src={model.preview_url} style={{ height: 32, flex: 1 }} />
                    ) : (
                      <div style={{ flex: 1, fontSize: 13, color: 'var(--fg-4)' }}>Audio unavailable</div>
                    )}
                    <span className="badge badge-purple" style={{ textTransform: 'capitalize' }}>Generated</span>
                  </div>
                ))}
              </div>
            </div>

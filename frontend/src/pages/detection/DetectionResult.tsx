    ...
    <div style={{ padding: '28px 32px', maxWidth: 1100, margin: '0 auto' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 340px', gap: 20 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <Reveal delay={0.07}>
            <div className="card" style={{ padding: 20 }}>
              <div style={{ fontWeight: 600, fontSize: 13, color: 'var(--fg)', marginBottom: 14 }}>Analysis Feedback</div>
              {!feedbackGiven ? (
                <div>
                  <p style={{ fontSize: 12.5, color: 'var(--fg-4)', marginBottom: 12 }}>Was this detection accurate?</p>
                      <div className="flex gap-3">
                        <button onClick={() => { 
                          detectionApi.submitFeedback(id as string, true).catch(console.error);
                          setFeedbackGiven(true); 
                          toast.success('Feedback recorded. Thank you!') 
                        }} className="flex-1 flex items-center justify-center gap-2 py-2 border border-surface-200 rounded-lg text-sm font-600 text-surface-700 hover:bg-emerald-50 hover:text-emerald-700 hover:border-emerald-200 transition-all cursor-pointer">
                          <ThumbsUp size={14} /> Yes, Correct
                        </button>
                        <button onClick={() => { 
                          detectionApi.submitFeedback(id as string, false).catch(console.error);
                          setFeedbackGiven(true); 
                          toast.success('Feedback recorded. Thank you!') 
                        }} className="flex-1 flex items-center justify-center gap-2 py-2 border border-surface-200 rounded-lg text-sm font-600 text-surface-700 hover:bg-rose-50 hover:text-rose-700 hover:border-rose-200 transition-all cursor-pointer">
                          <ThumbsDown size={14} /> No, False Positive
                        </button>
                      </div>
                </div>
              ) : (
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '12px', background: 'rgba(22, 163, 74, 0.1)', borderRadius: 8, color: '#16a34a', fontSize: 12.5, fontWeight: 500 }}>

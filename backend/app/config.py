"""
Vocaria Platform Configuration
...
class Settings(BaseSettings):
    ...
    # ── Application ─────────────────────────────────────────────────────────
    APP_NAME: str = "Vocaria"
    APP_VERSION: str = "1.0.0"
    ENVIRONMENT: str = "development"
    DEBUG: bool = False
    SECRET_KEY: str = "voice_crafter_development_secret_key_12345"
    ALGORITHM: str = "HS256"
    ...
    LOCAL_STORAGE_PATH: str = "/tmp/vocaria-storage"
    ...
    # TTS / Voice Generation (Kokoro-82M — Apache 2.0)
    TTS_MODEL: str = "kokoro"
    TTS_SAMPLE_RATE: int = 24000
    TTS_MAX_TEXT_LENGTH: int = 5000
    TTS_STREAMING_CHUNK_SIZE: int = 50  # tokens per streaming chunk
    TTS_DEFAULT_VOICE: str = "af_heart"  # Default Kokoro voice preset
    ...
    SMTP_FROM_EMAIL: str = "noreply@vocaria.ai"
    SMTP_FROM_NAME: str = "Vocaria"
    SMTP_TLS: bool = True
    ...
    @validator("SUPABASE_URL", "SUPABASE_SERVICE_KEY")
    def validate_supabase_config(cls, v):
        if not v:
            raise ValueError("SUPABASE_URL or SUPABASE_SERVICE_KEY is missing in .env! Backend cannot verify tokens without it.")
        return v

...
async def check_voice_profile_quota(user: User, db: AsyncSession):
    ...
    if count >= max_profiles:
        raise HTTPException(
            status_code=402,
            detail=f"You've hit your Voice Profile limit ({max_profiles})! Upgrade your plan today to unlock unlimited creativity and store as many premium voices as you need.",
        )
...
async def check_clone_quota(user: User, db: AsyncSession):
    ...
    if used >= max_jobs:
        raise HTTPException(
            status_code=402,
            detail=f"You've used all your free cloning credits for this month ({max_jobs} jobs)! Buy a premium plan now to instantly unlock more cloning power and scale your ideas.",
        )
...
async def check_generation_quota(user: User, char_count: int, db: AsyncSession):
    ...
    if used + char_count > max_chars:
        ...
        raise HTTPException(
            status_code=402,
            detail=f"Your free platform credits have been fully utilized! ({used + char_count:,} / {max_chars:,} characters). Upgrade to a premium plan right now to unlock massive generation limits and keep creating without interruptions!",
        )

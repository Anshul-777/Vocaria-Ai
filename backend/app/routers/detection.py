...
async def get_detection_stats(
    ...
    feedback_stats = await db.execute(
        select(
            ...
            DetectionJob.user_id == current_user.id,
            DetectionJob.status == JobStatus.COMPLETED,
            DetectionJob.user_feedback.isnot(None)
        )
    )
    ...
    return {
        ...
        "accuracy_matrix": {
            "true_authentic": fb_row.true_authentic or 0,
            "false_synthetic": fb_row.false_synthetic or 0,

"""Vocaria - Cloning, Generation, Hub, Plans, Analytics, Users, Orgs, APIKeys, Notifications, History, Audit, Quality, Benchmarks, Streaming, Uploads, Admin routers"""
...
async def analytics_overview(current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    ...
    failed_clones = (await db.execute(select(func.count(CloneJob.id)).where(CloneJob.user_id == current_user.id, CloneJob.status == "failed"))).scalar() or 0
    failed_generations = (await db.execute(select(func.count(GenerationJob.id)).where(GenerationJob.user_id == current_user.id, GenerationJob.status == "failed"))).scalar() or 0
    failed_detections = (await db.execute(select(func.count(DetectionJob.id)).where(DetectionJob.user_id == current_user.id, DetectionJob.status == "failed"))).scalar() or 0
    failed_total = failed_clones + failed_generations + failed_detections

    return {
        "totals": {"clone_jobs": clone_total, "generation_jobs": gen_total,
                   "detection_jobs": det_total, "voice_profiles": voice_total,
                   "synthetic_detected": synthetic_count, "failed_jobs": failed_total},
        "usage": usage["usage"],
...
async def analytics_timeline(
    ...
    credits_result = await db.execute(
        select(func.date(GenerationJob.created_at).label("day"), func.sum(GenerationJob.character_count).label("chars"))
        .where(GenerationJob.user_id == current_user.id, func.date(GenerationJob.created_at) >= start)
        .group_by(func.date(GenerationJob.created_at)).order_by("day")
    )
    credits_by_day = {str(r.day): int(r.chars or 0) for r in credits_result}

    # Failed jobs by day
    failed_gen = await db.execute(
        select(func.date(GenerationJob.created_at).label("day"), func.count().label("count"))
        .where(GenerationJob.user_id == current_user.id, func.date(GenerationJob.created_at) >= start, GenerationJob.status == "failed")
        .group_by(func.date(GenerationJob.created_at))
    )
    failed_gen_by_day = {str(r.day): r.count for r in failed_gen}

    failed_det = await db.execute(
        select(func.date(DetectionJob.created_at).label("day"), func.count().label("count"))
        .where(DetectionJob.user_id == current_user.id, func.date(DetectionJob.created_at) >= start, DetectionJob.status == "failed")
        .group_by(func.date(DetectionJob.created_at))
    )
    failed_det_by_day = {str(r.day): r.count for r in failed_det}

    failed_clone = await db.execute(
        select(func.date(CloneJob.created_at).label("day"), func.count().label("count"))
        .where(CloneJob.user_id == current_user.id, func.date(CloneJob.created_at) >= start, CloneJob.status == "failed")
        .group_by(func.date(CloneJob.created_at))
    )
    failed_clone_by_day = {str(r.day): r.count for r in failed_clone}
    ...
    while current_day <= end:
        ...
        timeline.append({
            "date": day_str,
            "generations": gen_by_day.get(day_str, 0),
            "detections": det_by_day.get(day_str, 0),
            "credits_used": credits_by_day.get(day_str, 0),
            "failed_jobs": failed_gen_by_day.get(day_str, 0) + failed_det_by_day.get(day_str, 0) + failed_clone_by_day.get(day_str, 0)
        })
...
async def get_history(
    page: int = 1, page_size: int = 20,
    job_type: Optional[str] = Query(None, pattern="^(clone|generation|detection|profile|live|quality|agent)$"),
    current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db),
):
    ...
    if not job_type or job_type == "profile":
        ...
        for v in res.scalars().all():
            items.append({"type": "profile", "id": v.id, "status": "completed", "summary": f"Created voice: {v.name}",
                          "language": v.language, "gender": v.gender, "created_at": v.created_at})
    if not job_type or job_type == "live":
    ...
    if not job_type or job_type == "quality":
        res = await db.execute(
            select(QualityAnalysis, UploadedFile.original_filename)
            .join(UploadedFile, QualityAnalysis.file_id == UploadedFile.id)
            .where(UploadedFile.user_id == current_user.id)
            .order_by(desc(QualityAnalysis.created_at))
            .limit(page_size)
        )
    ...
    if not job_type or job_type == "agent":
        voices_res = await db.execute(select(VoiceProfile.name).where(VoiceProfile.owner_id == current_user.id, VoiceProfile.is_active == True))
        voice_names = list(voices_res.scalars().all())
        if not voice_names:
            voice_names = ["Cloned Support Agent", "Assistant Clara"]
        import random
        random.seed(current_user.id)
        for i, name in enumerate(voice_names[:4]):
            dt = datetime.now(timezone.utc) - timedelta(days=i*2, hours=i*3)
...
async def analyze_audio_quality(
    ...
    try:
        ...
        try:
            storage = await get_storage()
            storage_key = f"quality_analyses/{current_user.id}/{uuid.uuid4()}_{file.filename}"
            await storage.upload(settings.BUCKET_UPLOADS, storage_key, content, file.content_type or "audio/wav")
            ...
            logger.warning(f"Could not persist quality analysis: {db_err}")
...
async def serve_file(bucket: str, key: str):
    import os
    from app.config import settings
    # Resolve the absolute path to make sure we don't have CWD issues
    base_path = os.path.abspath(settings.LOCAL_STORAGE_PATH)
    storage_path = os.path.join(base_path, bucket, key)
